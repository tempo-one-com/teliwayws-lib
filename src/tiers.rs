...
use crate::{extract_xml_tag, error::{Error, Result}};
...
impl TryFrom<String> for TiersWsResponse {
    type Error = Error;
    
    fn try_from(value: String) -> Result<Self> {
        let tag = "resultatCreationModificationTiers";        
        let xml = extract_xml_tag(tag, &value).ok_or(Error::Parsing(tag.to_string()))?;
        let data: core::result::Result<TiersWsResponse, quick_xml::DeError> = quick_xml::de::from_str(xml);
...
mod tests {
    ...
    fn extractor_embedded() {
        ...
        assert_eq!(part, Some(r#"<resultatCreationModificationTiers xsi:type="ns1:Tiers"><idTiers xsi:type="xsd:int">599</idTiers><sCode xsi:type="xsd:string">10001</sCode></resultatCreationModificationTiers>"#))
    }
    ...
    fn into_struct() {
      ...
      let s: core::result::Result<TiersWsResponse,_> = text.to_owned().try_into();
