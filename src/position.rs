...
impl TryFrom<String> for PositionEventMarkerWsResponse {
    type Error = Error;
    
    fn try_from(value: String) -> Result<Self> {
        let tag = "tabResultatsPointagePosition";        
        let xml = extract_xml_tag(tag, &value)
            .ok_or(Error::Parsing(tag.to_string()))?;

        let data: core::result::Result<PositionTagList, quick_xml::DeError> = quick_xml::de::from_str(xml);

        match data {
            Ok(x) => {
                let positions = x.items.into_iter().map(|x| x.into()).collect::<Vec<_>>();
                
                Ok(PositionEventMarkerWsResponse {
                    positions,
                })
            },
...
impl PositionTag {
    pub fn get_recepisse(&self) -> Result<String> {
        ...
        let dot = self.label.find('.')
            .ok_or(Error::MissingParam(".".to_string()))?;
        let du = self.label.find("du")
